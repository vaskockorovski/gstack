...
# `<phase>` is `loop` when the user passed --loop, `final_gate` otherwise.
# SET THIS LINE before running the block: `loop` if the user typed `/codex review --loop`,
# `final_gate` for a default or focused review, and `none` for challenge/consult/plan modes,
...
_OV_PHASE=final_gate    # ← loop | final_gate | none
_NEEDS_CODEX=yes
