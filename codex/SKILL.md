...
_OV_OUT=$(mktemp "$TMP_ROOT/gstack-ov-out-XXXXXX.txt")
...
# 55 x 10s = 550s, inside the host's 600000 maximum with headroom. Polling for THIRTY minutes
# under a TEN minute cap was arithmetic that could not hold: the host killed the wrapper before
# the exit code and findings were emitted, every time a round ran long — which is the case this
# path exists for. A round that outlives the window is reported as ROUND STATE UNKNOWN, which
# is honest, and the orphan note below says how to find the child that is still billing.
...
if [ -s "$_OV_DONE" ]; then
  ...
  _OV_EXIT=125
  echo "ROUND STATE UNKNOWN — the review did not finish within the 30-minute poll and may still be running. If neither gtimeout nor timeout is installed the adapter warned that it ran UNCAPPED, in which case the call is not bounded by anything and is now orphaned: find it with 'ps -eo pid,etime,cmd | grep gstack-outside-voice' and kill it by PID before re-running, or it keeps billing. This is NOT a clean round and NOT a timeout; nothing has been established. The findings file is left in place for the running call."
fi
rm -f "$_LOOP_PROMPT"
...
[ "$_OV_EXIT" = "125" ] || rm -f "$_OV_FINDINGS" "$_OV_OUT"
```
