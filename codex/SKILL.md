...
If the output contains `AUTH_FAILED`, **first check whether Codex is actually needed.**
This preflight predates the outside-voice adapter and stops on missing Codex auth
unconditionally, which makes the whole feature unreachable on a machine configured for a
hosted backend — `/codex review --loop` with `outside_voice_loop=openrouter`, and a gate
with `outside_voice_gate=openrouter`, both need no Codex at all and were refused up front
anyway:

```bash
_NEEDS_CODEX=no
for _ph in loop final_gate; do
  [ "$(~/.claude/skills/gstack/bin/gstack-outside-voice backend --phase $_ph 2>/dev/null)" = "codex" ] && _NEEDS_CODEX=yes
done
echo "NEEDS_CODEX: $_NEEDS_CODEX"
```

If `NEEDS_CODEX: no`, do NOT stop — no phase routes to Codex, so its absence is not a
blocker. Note it and continue. Only when a phase does route to Codex, stop and tell the user:
...
PROMPT
_OV_FINDINGS="$TMP_ROOT/gstack-ov-findings.json"
...
_OV_DONE="$TMP_ROOT/gstack-ov-done"
_OV_LAUNCHED="$TMP_ROOT/gstack-ov-launched"
...
_OV_OUT="$TMP_ROOT/gstack-ov-out.txt"
