...
elif [ "$_CODEX_EXIT" = "3" ]; then
  # Outside-voice review is switched off. Reachable here only because this path now routes
  # through the adapter: raw `codex` never returns 3, so the generic branch below — written
  # when it did — reported a deliberate user setting as "[codex exit 3]" and logged it as
  # codex_nonzero_exit, i.e. a config choice rendered as a tool failure in both the message
  # and the telemetry. Not an error, and emphatically NOT a clean gate: nothing was reviewed.
  echo "GATE NOT RUN — outside-voice review is disabled. This is NOT a clean gate; nothing was reviewed. Re-enable: \`gstack-config set codex_reviews enabled\`."
...
~/.claude/skills/gstack/bin/gstack-outside-voice exec \
  ...
  --base "origin/<base>" --effort high --timeout 330 \
