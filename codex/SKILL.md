...
Once you have read the counts, delete it — this is a step, not a suggestion:

```bash
rm -f "$_OV_FINDINGS"
```

It is a tempfile per round and nothing else cleans it up. The prompt file is removed
immediately after the call; the findings file has to survive until it is read, which
is exactly how it was missed. Stating it in prose was not enough either — the round
after that fix still reported the leak, because a cleanup that is not in a code block
does not get run.
