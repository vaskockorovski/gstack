...
if [ "$_OV_PHASE" = "auto" ]; then
  # THE SAME QUESTION THE EXEC WILL ASK. resolve-phase defaults to origin/main and to
  # non-explicit mode; the review below runs `exec --phase auto --explicit --base origin/<base>`.
  # Omitting either flag lets this resolution disagree with the phase the adapter actually picks —
  # on any branch whose base is not main, or when codex_reviews=disabled and the user asked for a
...
if [ "$_OV_EXIT" = "125" ]; then
  echo "PHASE_THAT_RAN: unknown — the round has NOT finished. Any promotion notice above belongs to a gate half that is still executing; re-run the poll block. Nothing here is a verdict."
else
_OV_RAN_PHASE="$_OV_RESOLVED_PHASE"
_OV_ANNOUNCED=$(sed -n "s/.*phase resolved to '\([a-z_]*\)'.*/\1/p" "$TMPERR" 2>/dev/null | tail -1)
if [ -n "$_OV_ANNOUNCED" ]; then
  [ "$_OV_ANNOUNCED" != "$_OV_RESOLVED_PHASE" ] && echo "NOTE: the ledger moved between the pre-call probe ($_OV_RESOLVED_PHASE) and the round the adapter actually started ($_OV_ANNOUNCED). The announcement wins; effort and budget were chosen from the probe and are stated above."
  _OV_RAN_PHASE="$_OV_ANNOUNCED"
fi
...
[ "$_OV_EXIT" = "125" ] || echo "PHASE_THAT_RAN: $_OV_RAN_PHASE"
...
# admitted every failing exit. `_OV_RAN_PHASE` comes from an announcement emitted BEFORE the
...
if [ "$_OV_EXIT" = "0" ] && [ "$_OV_RAN_PHASE" = "final_gate" ]; then
