...
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
cd "$_REPO_ROOT"
codex review "${CODEX_BOUNDARY}Review the changes on this branch against the base branch <base>. Run git diff origin/<base>...HEAD 2>/dev/null || git diff <base>...HEAD to see the diff and review only those changes." -c 'model_reasoning_effort="high"' $(~/.claude/skills/gstack/bin/gstack-codex-search-flag) < /dev/null 2>"$TMPERR"
\`\`\`
...
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
codex exec "<prompt>" -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="high"' $(~/.claude/skills/gstack/bin/gstack-codex-search-flag) < /dev/null 2>"$TMPERR_PV"
\`\`\`
...
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
codex exec "<prompt>" -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="high"' $(~/.claude/skills/gstack/bin/gstack-codex-search-flag) < /dev/null 2>"$TMPERR_DOC"
\`\`\`
